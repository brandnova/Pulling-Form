...
def template_preview(request, template_id):
    try:
        template = CustomFormTemplate.objects.get(id=template_id)
        # Create a dummy form for preview purposes
        dummy_form = PublicFormSubmissionForm()
        
        # Render the template with the dummy form
        custom_template = Template(template.html_content)
        context = Context({'form': dummy_form})
        html_content = custom_template.render(context)
        
        # Inject custom CSS
        css_content = f"<style>{template.css_content}</style>"
        full_content = f"{css_content}{html_content}"
